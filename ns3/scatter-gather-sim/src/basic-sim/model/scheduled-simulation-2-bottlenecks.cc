#include "scheduled-simulation-2-bottlenecks.h"
#include <cmath>
#include "ns3/random-variable-stream.h"

namespace ns3 {
    int scheduled_simulation_2_bottlenecks(std::string run_dir) {
        ...
        while (number_of_workers <= simulationConfig.workers_end) {
            ...
            worker_subnet2.SetBase("10.8.1.0", "255.255.255.0");
            ...
            Ipv4AddressHelper frontend_subnet6;
            frontend_subnet6.SetBase("10.7.1.0", "255.255.255.0");
            ...
            NetDeviceContainer switch_frontend_nd2 = switch_frontend.Install(
                    NodeContainer(switches.Get(1), nodes.Get(1)));

            NetDeviceContainer switch_0_2 = switch_frontend.Install(
                    NodeContainer(switches.Get(0), switches.Get(2)));

            NetDeviceContainer switch_1_2 = switch_frontend.Install(
                    NodeContainer(switches.Get(1), switches.Get(2)));

            NetDeviceContainer switch_3_2 = switch_frontend.Install(
                    NodeContainer(switches.Get(3), switches.Get(2)));

            NetDeviceContainer switch_4_2 = switch_frontend.Install(
                    NodeContainer(switches.Get(4), switches.Get(2)));
            ...
            tch.Install (switch_0_2);
            tch.Install (switch_1_2);
            tch.Install (switch_3_2);
            tch.Install (switch_4_2);
            ...
            for (int i = number_of_workers + 2; i < 2*number_of_workers + 2; i++) {
                NetDeviceContainer link = p2p.Install(NodeContainer(nodes.Get(i), switches.Get(4)));
            ...
            frontend_subnet3.Assign(switch_0_2);
            frontend_subnet4.Assign(switch_1_2);
            frontend_subnet5.Assign(switch_3_2);
            frontend_subnet6.Assign(switch_4_2);
